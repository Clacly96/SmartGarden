...
def create_rule(event_client,ev_data,rule_type):
    ...
    event_response = event_client.put_rule(
        Name=str(ev_data['Keys']['plantUUID']['S'])+'_{0}'.format(rule_type),
...
def del_event_rule(lambda_arn,lambda_client,event_client,ev_data):
    ...
    event_client.remove_targets(
        Rule=str(ev_data['Keys']['plantUUID']['S'])+'_begin',
        Ids=['1']
    )
    #remove end rule target
    event_client.remove_targets(
        Rule=str(ev_data['Keys']['plantUUID']['S'])+'_end',
        Ids=['1']
    )
    #delete begin rule
    event_client.delete_rule(
        Name=str(ev_data['Keys']['plantUUID']['S'])+'_begin'
    )
    #delete end rule
    event_client.delete_rule(
        Name=str(ev_data['Keys']['plantUUID']['S'])+'_end'
    )
    #remove begin rule permission
    lambda_client.remove_permission(
        FunctionName=lambda_arn,
        StatementId=str(ev_data['Keys']['plantUUID']['S'])+'_begin_lambda'
    )
    #remove end rule permission
    lambda_client.remove_permission(
        FunctionName=lambda_arn,
        StatementId=str(ev_data['Keys']['plantUUID']['S'])+'_end_lambda'
    )

def lambda_handler(event, context):
    #set arn of the destination lambda function
    destination_lambda_arn='arn:aws:lambda:[country]:[account-ID]:function:publish_event_tweet'
